...
def create(path, template):
    ...
    click.echo(f"  Edit {car_path}/myconfig.py to configure your car settings.")
...
def configure(car_path):
    ...
    record_button = click.prompt("Record button code (or -1 to skip)", type=int, default=-1)
    mode_button = click.prompt("Mode button code (or -1 to skip)", type=int, default=-1)
