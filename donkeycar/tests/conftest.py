...
# Turn on our deterministic helpers early for the test process.
try:
    from donkeycar.deterministic import enable_determinism

    enable_determinism(1234)
except Exception:
    # If determinism helper or backends are missing, continue without failing tests.
    pass
...
@pytest.fixture
def tub_path_tmp(tmpdir):
    '''Create a temporary tub path.'''
    path = tmpdir.mkdir('tubs').join('tub')
    return str(path)


@pytest.fixture
def tub(tub_path_tmp):
    t = create_sample_tub(tub_path_tmp, records=128)
    return t


@pytest.fixture
def tubs(tmpdir, num_tubs=5):
    tubs_dir = tmpdir.mkdir('tubs')
    tub_paths = [str(tubs_dir.join('tub_{}'.format(i)))
                 for i in range(num_tubs)]
    tub_objs = [create_sample_tub(tub_path, records=5)
                for tub_path in tub_paths]
    return str(tubs_dir), tub_paths, tub_objs
